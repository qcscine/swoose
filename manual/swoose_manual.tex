...
%%
% Book metadata
\title[SCINE Swoose manual]{User Manual \vskip 0.5em {\setlength{\parindent}{0pt} \Huge SCINE Swoose 3.0.0}}
...
\item \hl{export\_SFAM\_for\_openMM}: Decides whether to export the SFAM functional form and its parameters in the XML file format, which can be interfaced with OpenMM.\cite{eastman2017} The default is set to \textit{false}.
Note that the SFAM does not use residues. Therefore, each atom type is its own residue and the residue mapping must be provided explicitly to OpenMM.
\item \hl{sfam\_openmm\_file\_name}: The name of the XML file to which the parameters are exported for OpenMM.\cite{eastman2017} The default is set to \textit{./sfam.xml}.
\item \hl{sfam\_atom\_type\_file}: Path to the atom type file to be used by SFAM. The default is set to \textit{""}. If an atom type file is given, the atom types are written to this file after atom type construction. If \textit{sfam\_atom\_types\_from\_file} is true, the atom types in the file are used instead of constructing them on the fly.
\item \item \hl{sfam\_atom\_types\_from\_file}: If true, atom types are read from file. An atom type file must also be provided (see \textit{sfam\_atom\_type\_file}).
\end{itemize}
...
\item \hl{openmm\_xml\_files}: List of OpenMM-style XML parameter files. This file can contain any Amber force field. Use this feature together with the gaff\_atom\_types\_file.
\end{itemize}
...
\begin{itemize}
    \item \hl{excluded\_residue\_labels}: Atoms belonging to residues with labels given in this list will be excluded from the QM region.
\end{itemize}
